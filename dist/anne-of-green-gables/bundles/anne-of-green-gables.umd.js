(function (global, factory) {
    ...
    var AnneOfGreenGablesComponent = /** @class */ (function () {
        function AnneOfGreenGablesComponent() {
            ...
            this.counterMove = new i0.EventEmitter();
            this.selectedMarkerlng = new i0.EventEmitter();
        ...
        AnneOfGreenGablesComponent.prototype.initMap = function () {
            ...
            this._map.on('locationfound', function (e) {
                _this.layerTemp = leafletNeshan.circle(e.latlng, e.accuracy - 50).addTo(e.target);
                _this.circleLayerGroup = leafletNeshan.layerGroup().addLayer(_this.layerTemp).addTo(e.target);
                _this.currentLocation = e.latlng;
            });
        };
        AnneOfGreenGablesComponent.prototype._emitLastCenter = function () {
            var _this = this;
            this._map.on('moveend', function (_ev) {
                _this.lastCenter.emit(_this._map.getCenter());
                _this.counter++;
                _this.counterMove.emit(_this.counter);
            });
        };
        AnneOfGreenGablesComponent.prototype.concatMarkers = function (markers) {
            var _this = this;
            markers.forEach(function (item) {
                leafletNeshan.marker([item.lat, item.lng], { icon: _this.unselectedIcon }).on('click', function (ev) {
                    _this.Layers = _this.layerGroup.getLayers();
                    _this.Layers.forEach(function (element) {
                        if (element instanceof leafletNeshan.Marker) {
                            element.setIcon(_this.unselectedIcon);
                        }
                    });
                    ev.target.setIcon(_this.selectedIcon);
        ...
        AnneOfGreenGablesComponent.prototype.onLocationFound = function (e) {
            var layerTemp = leafletNeshan.circle(e.latlng, e.accuracy - 50).addTo(e.target);
        ...
        AnneOfGreenGablesComponent.prototype.selectMarker = function (item) {
            this.layerGroup.getLayers().find(function (x) { return x.lat === item.lat && x.lng === item.lng; }).setIcon(this.selectedIcon);
        };
        AnneOfGreenGablesComponent.prototype.unSelectMarker = function (item) {
            this.layerGroup.getLayers().find(function (x) { return x.lat === item.lat && x.lng === item.lng; }).setIcon(this.unselectedIcon);
        };
